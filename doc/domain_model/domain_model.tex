...
utilizes the above concepts to map a field from one mesh to another.

A linear triangle mesh is chosen for the source mesh as shown in
Fig.~\ref{fig:source_mesh}. The source mesh is decomposed over 4
parallel processes with each process operating on elements of a single
color. The targt mesh is defined with linear quadrilaterals and is
given by Fig.~\ref{fig:target_mesh}. Here, the decomposition is also
over 4 processes but with a different spatial decomposition.

\begin{figure}[htpb!]
  \centering
  \includegraphics[width=5.5in]{tri_part.png}
  \caption{\sl Source mesh for 2D shared domain example.}{\sl Each
...
\begin{figure}[htpb!]
  ...
  \caption{\sl Target mesh for 2D shared domain example.}{\sl Each
    color represents the portion of the mesh owned by each parallel
    process.}
  \label{fig:target_mesh}
\end{figure}

Shown in Fig~\ref{fig:source_field}, we define a field over the source
mesh, in this case the peaks function, to transfer to the target mesh.

\begin{figure}[htpb!]
  \centering
  \includegraphics[width=5.5in]{tri_peaks.png}
  \caption{\sl Source function for 2D shared domain example.}{\sl A 2D
    peaks function is defined over the source domain.}
...
as shown in Fig.~\ref{fig:rendezvous_part}.

\begin{figure}[htpb!]
  \centering
  \includegraphics[width=5.5in]{tri_rend.png}
  \caption{\sl Rendezvous decomposition for 2D shared domain
    example.}{\sl Both the source and target geometries are
    repartitioned in this fashion such that all search operations are
    on-process.}
  \label{fig:rendezvous_part}
\end{figure}

Finally, an evaluator associated with the source mesh is used to drive
peaks function evaluations on the target mesh nodes. These are then
communicated back to the target decomposition, resulting in the field
transferred as shown in Fig~\ref{fig:target_field}.

\begin{figure}[htpb!]
  \centering
  \includegraphics[width=5.5in]{quad_peaks.png}
  \caption{\sl Target function for 2D shared domain example.}{\sl A 2D
    peaks function is transferred to the target domain.}
  \label{fig:source_field}
\end{figure}
