...
\item {\bf Mesh:} An discrete representation of the $n$-dimensional
  ...
  number of {\sl mesh elements}, $\omega$. The mesh elements that
  construct a mesh {\sl may not intersect}. Mesh can be considered a
  subset of geometry.
...
construct them. This subset has been demonstrated as sufficient for
...
will contain the unique vertex global ordinals that construct it. An
element may be repeated any number of times across the parallel domain
...
numbering scheme and the client canonical numbering scheme. A
permutation list must be described globally, regardless of whether or
not elements exist on a particular process. For elements that have a
higher order basis (e.g. quadratic), DTK resolves these as higher
order vertices via the MBCN system. See Appendix~\ref{apdx:cell_topo}
for canonical element topologies as defined by DTK.
...
Global recursive coordinate bisectioning parameters are maintained for
global partitioning information. A local kD-tree is formed over the
local mesh. In this way, mesh searches on the global level use the RCB
partitioning information and mesh searches on the local level use the
kD-tree to generate a subset of the mesh in the vicinity of the target
point. This subset is then searched with a more expensive
...
iteration strategy to determine if the vertex is contained within.
