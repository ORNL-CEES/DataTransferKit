...
\begin{equation}
  f_{\Omega} = \frac{1}{m_{\Omega}} \sum_i m_{{\omega}_i}
  f_{{\omega}_i},\ \forall \omega_i \in \Omega\:,
  \label{eq:integral_sum}
\end{equation}
where $m_{{\omega}_i}$ is the measure of the $i^{th}$ mesh element and
$m_{\Omega}$ is the measure of the geometric domain.
...
\begin{equation}
  f_{\Omega} = \frac{\sum_i \Big[m_{{\omega}_i} \int_{{\omega}_i} F(r)
      dr \Big]}{m_{\Omega}}\:,
  \label{eq:element_weighted_int}
\end{equation}
where $m_{{\omega}_i}$ is the measure of the $i^{th}$ mesh element and
$m_{\Omega}$ is the measure of the target geometry. If the mesh is
assumed conformal to the geometry then:
\begin{equation}
