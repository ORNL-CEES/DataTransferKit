...
\item {\bf Mesh:} An $n$-dimensional discrete representation of the
  ...
  subset of geometry.
\item {\bf Field:} A $D$-dimensional discrete representaion of a
...
\item {\bf Evaluator:} An object that evaluates a field at a physical
  location, $\hat{r}$, in the spatial domain $\Omega$.
...
\item {\bf Data Space:} An empty field, $G$, of dimension $D$, to
  which data has yet to be applied such that $G(r) : \mathbb{R}^n
  \rightarrow \mathbb{R}^D, r \in \mathbb{R}^n$.
...
\item{ \bf Parallel Topology Map:} An operator, $M$, that defines the
  translation of a field from a source spatial domain to a target
  spatial domain such that $G(t) \leftarrow M( F(s) ), \forall t \in
  \Omega_T, \forall s \in \Omega_S$, using both geometric and parallel
  operations.
\item {\bf Transfer Operator:} An operator that uses an evaluator to
  compute a source field, $F(s)$, and applies a mesh map to fill a
  target data space, $G(t)$.
\end{itemize}
...
communicator ($C_{union}$) as a subset of the global communicator
($C_{global}$) such that $C_{union} = A \cup B$ and $C_{union} \in
C_{global}$. This operation is only valid if $A \in C_{global}$ and $B
\in C_{global}$.
...
where $\alpha \in \mathbb{R}$ and $g_d$ is the resulting scaled field.
...
\subsection{Conformal Domains}\label{subsec:map_algorithm}
...
\subsection{Non-Conformal Domains}
Currently, neither this domain model nor DTK consider non-conformal
domains for generating geometric mappings.
