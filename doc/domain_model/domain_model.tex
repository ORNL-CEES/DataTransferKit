...
defined as length in 1 dimension, area in 2 dimensions, and volume for
...
\end{equation}
where the integral over $\Omega$ will be the summation of all element
integrals:
\begin{equation}
  f_{\Omega} = \sum_i f_{{\omega}_i},\ \forall \omega_i \in \Omega\:.
...
target geometry cannot be used to generate a shared domain map). There
are 3 types of shared domain mapping algorithms implemented in DTK:
the traditional mesh-based rendezvous algorithm, the integral assembly
map, and the geometry source map.
...
instead of the mesh. There are two potential use cases for this type
of map: one in which the target is a set of points at which the source
...
array of general geometric structures is searched. The result of the
mapping sequence is a set of geometry/target point pairs used to drive
the evaluation algorithms for the source functions.
...
parallel decompositions of the geometries are not known a priori.
...
domain, then this averaged quantity will require some sort of measure
weighted integral to be performed over the source field defined on the
mesh elements and approriately assembled in parallel over the proper
