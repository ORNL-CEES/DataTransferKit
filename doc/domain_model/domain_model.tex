...
\begin{abstract}
  ...
  arbitrary physics components. In many physics applications, the
  concept of mesh is used to subdivide the physical domain into a
  discrete representation to facilitate the solution of the model
  problems that describe it. Additionally, the concept of the field is
  used to apply degrees of freedom to the mesh as a means of function
  discretization. With the increased development efforts in
  multiphysics simulation, adaptive mesh simulations, and other
...
\item {\bf Degrees of Freedom:} Discrete values generated by a physics
  computation that describe the discretization of quantity over phase
  space.
...
construct its linear form. An element may be repeated any number of
times across the parallel domain (i.e. it may have unlimited local
instances), however, every globally unique ordinal must have the same
connectivity list associated with it. For consistency, DTK uses the
MoaB Canonical Numbering (MBCN) scheme as a canonical ordering scheme
...
relationship between the source and target geometries is defined
through the same set of shared global ordinals. In this context, a
representation of the same geometric object exists in both the source
and target codes, but their parallel decomposition may be different
for physics-related reasons. These objects are also characterized by
the same group of global ordinals that provide their relationship.
