...
TEUCHOS_UNIT_TEST( VolumeSourceMap, cylinder_test )
{
    ...
    if ( my_rank == 0 )
    {
	target_field = Teuchos::rcp( new MyField( num_target_points, target_dim ) );
	target_coords = Teuchos::rcp( new MyField( num_target_points, geom_dim ) );
	...
	target_coords =	Teuchos::rcp( new MyField( 0, geom_dim ) );
    }
    comm->barrier();
	...
	comm, source_geometry_manager->dim(), true, 1.0e-6 );
