...
TEUCHOS_UNIT_TEST_TEMPLATE_1_DECL( ExodusAsciiGenerator, one_to_one, Node )
{
    ...
    auto generator =
        std::make_shared<DataTransferKit::ExodusAsciiGenerator<DeviceType,DeviceType> >(
            comm,
            src_coord_file, src_connectivity_file,
            tgt_coord_file, tgt_connectivity_file );

    // Generate a problem.
    Kokkos::View<DataTransferKit::Coordinate**,Kokkos::LayoutLeft,DeviceType> src_coords;
    Kokkos::View<DataTransferKit::Coordinate**,Kokkos::LayoutLeft,DeviceType> tgt_coords;
    generator->createOneToOneProblem( src_coords, tgt_coords );

    // Print sizes.
    std::cout << comm->getRank() << " "
              << src_coords.extent(0) << " " << src_coords.extent(1) << " "
              << tgt_coords.extent(0) << " " << tgt_coords.extent(1) << std::endl;
}
