...
createGhostedMap( const Teuchos::RCP<const Teuchos::Comm<int>> &comm,
                  const GO global_num_elements, const int num_overlap )
{
    ...
    return Tpetra::createNonContigMapWithNode<LO, GO, NO>( node_global_ids,
                                                           comm );
}
...
TEUCHOS_UNIT_TEST_TEMPLATE_1_DECL( TpetraImportExport, ghosted_to_unique, Node )
{
    ...
    SC vec_norm_1 = vec_2->norm1();
    SC test_norm_1 =
        num_global * value + ( comm->getSize() - 1 ) * 2 * num_overlap;
    TEST_EQUALITY( vec_norm_1, test_norm_1 );
...
TEUCHOS_UNIT_TEST_TEMPLATE_1_DECL( TpetraImportExport, ghosted_to_ghosted,
                                   Node )
{
    // Template aliases.
    using SC = double;
    ...
    int num_overlap_2 = 8;
    auto map_1 =
        createGhostedMap<LO, GO, NO>( comm, num_global, num_overlap_1 );
    auto map_2 =
        createGhostedMap<LO, GO, NO>( comm, num_global, num_overlap_2 );
    ...
    auto tpetra_export_1_to_unique =
        Tpetra::createExport( map_1, unique_map_2 );

    // Create an import object from unique to map 2.
    auto tpetra_import_unique_to_2 =
        Tpetra::createImport( unique_map_2, map_2 );
    ...
    // Import unique to 2.
    vec_2->doImport( *unique_vec_2, *tpetra_import_unique_to_2,
                     Tpetra::REPLACE );
...
#define UNIT_TEST_GROUP( NODE )                                                \
    TEUCHOS_UNIT_TEST_TEMPLATE_1_INSTANT( TpetraImportExport,                  \
                                          unique_to_unique, NODE )             \
    TEUCHOS_UNIT_TEST_TEMPLATE_1_INSTANT( TpetraImportExport,                  \
                                          ghosted_to_unique, NODE )            \
    TEUCHOS_UNIT_TEST_TEMPLATE_1_INSTANT( TpetraImportExport,                  \
                                          unique_to_ghosted, NODE )            \
    TEUCHOS_UNIT_TEST_TEMPLATE_1_INSTANT( TpetraImportExport,                  \
                                          ghosted_to_ghosted, NODE )
