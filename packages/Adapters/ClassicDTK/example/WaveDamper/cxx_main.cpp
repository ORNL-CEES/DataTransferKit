...
int main( int argc, char *argv[] )
{
    ...
    try
    {
        ...
        if ( damper_exists )
        {
            ...
            double damper_local_size =
                ( global_max - global_min ) / my_damper_size;
            double my_damper_min =
                my_damper_rank * damper_local_size + global_min;
            double my_damper_max =
                ( my_damper_rank + 1 ) * damper_local_size + global_min;
            int damper_num_local_elements = 11;

            // Create a Damper.
            damper = Teuchos::rcp( new Damper( damper_comm, my_damper_min,
                                               my_damper_max,
        ...
        // Setup the damper-to-wave map with the damper mesh as the source and
        // the
