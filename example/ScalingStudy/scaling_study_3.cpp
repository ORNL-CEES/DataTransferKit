//---------------------------------------------------------------------------//
/*!
 * \file scaling_study_3.cpp
 * \author Stuart R. Slattery
 * \brief Scaling study 3. Weak scaling in 3D over cube of elements with
					    ...
					    int num_points, int edge_size )
{
	...
	*(coordinate_field->begin() + i) = 
	    num_blocks * (edge_size-1) * (double) std::rand() / RAND_MAX;
...
int main(int argc, char* argv[])
{
    ...
    {
	int i_block = std::floor( 
	    target_coord_manager->field()->getData()[n] / (edge_size-1) );
	int j_block = std::floor(
	    target_coord_manager->field()->getData()[n+num_points] / (edge_size-1) );
	int k_block = std::floor(
	    target_coord_manager->field()->getData()[n+2*num_points] / (edge_size-1) );
