...
TEUCHOS_UNIT_TEST( SharedDomainMap, shared_domain_map_test2 )
{
	...
	new FieldManager<MyField>( MyField( field_size, 3 ), comm ) );
	...
	TEST_ASSERT( source_rank+1 == 
		     target_space_manager->field().getData()[n + target_dim_size] );
	TEST_ASSERT( source_rank+1 == 
		     target_space_manager->field().getData()[n + 2*target_dim_size] );
    }
}
