...
MyMesh buildMyMesh( int my_rank, int my_size, int edge_length )
{
    ...
    Teuchos::Array<long int> wedge_handles( num_elements );
    Teuchos::Array<long int> wedge_connectivity( 6*num_elements );
    ...
    Teuchos::Array<std::size_t> permutation_list( 6 );
    for ( int i = 0; i < permutation_list.size(); ++i )
    {
	permutation_list[i] = i;
    }

    return MyMesh( node_handles, coords, wedge_handles, wedge_connectivity,
		   permutation_list );
}
